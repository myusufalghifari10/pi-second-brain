...
| Knowledge dir resolution honors `PI_KNOWLEDGE_DIR` / `OMP_KNOWLEDGE_DIR` | `src/storage/sqlite.ts` (`getDefaultKnowledgeDir`), `docs/configuration.md:9-10` | Cache dir = `<knowledge-dir>/pdf-cache/`, same env plumbing |
...
- Cache lives under the knowledge dir ⇒ inherits `PI_KNOWLEDGE_DIR` overrides (module imports `getDefaultKnowledgeDir` from `../storage/sqlite.ts` — indexer→storage import precedent already exists in `chunker.ts`). Cache is content-addressed and shared across KBs; `knowledge_remove` does NOT delete it. Pruning: document `rm -rf <knowledge-dir>/pdf-cache` as the reset hatch. Unbounded growth is accepted and documented (one `.md` per distinct PDF; text is tiny vs. the PDF itself). **Implementation refinement (ratified at review): the key hashes the RESOLVED converter (auto-selected engine), not the raw config value — deterministic and collision-equivalent; U4 covers all bust cases.**
...
- **Blast-radius note (deviation from Layer 1's no-engine guarantee, deliberate):** Layer 2's subject IS the extraction path, which lives in `engine.ts`. Allowed engine.ts surface: the `.pdf` branch, `ExtractedSourceFile`, one skipped-reason string, plus ONE conditional `extraMetadata` argument at each of the three existing `analyzeIndexableContent(...)` call sites (**ratified Amendment A, 2026-09-18** — the converter-provenance carriage path, see §4.2), plus the two `searchBM25(...)` call arguments in `engine.search` switched from pre-normalized to raw query (**ratified Amendment B, 2026-09-18**: the engine-side pre-normalization double-normalizes — `bm25.ts` `prepareFtsTerms` normalizes again — which destroys Layer 1 composite canonical tokens (`mc²` → `mc pow2` → `mc pow`); E1 is the first engine-level test to expose this pre-existing defect; `searchBM25` already normalizes internally, so the pre-normalization was redundant and destructive). Nothing else in `engine.ts` may change (search, update loop, vectors, storage stay untouched — update loop benefits automatically because it calls `extractScannableFileContent`).

### 4.2 `chunkFile` + `analyzeIndexableContent` + `chunkMarkdown` threading

- `chunkFile(content, filePath, options?: { fileTypeOverride?: string; extraMetadata?: ChunkMetadata })`: when `options.fileTypeOverride` is set AND names a routing type (`markdown` | `latex`) that differs from extension detection, route using the override (`"markdown"` → `chunkMarkdown`, `"latex"` → `chunkLaTeX`) and forward `options.extraMetadata` into `chunkMarkdown`'s `extraMetadata` param; `extraMetadata` is always forwarded. **Amendment B refinement (2026-09-18):** the override is NOT forwarded when it names a non-routing type (`text`, `pdf`) — unconditional forwarding let extraction-level `"text"` demote extension-detected markdown routing and regress engine ranking behavior; when absent or not routing, behavior is byte-identical to today.
- `analyzeIndexableContent` passes its `fileType` param down: `chunkFile(content, filePath, { fileTypeOverride: analysisFileType, extraMetadata })` — the params already flow in from the call sites (`engine.ts:1123`, directory scan, update loop). `analyzeIndexableContent` gains an optional 4th param `extraMetadata?: ChunkMetadata` (default `undefined`). **Ratified Amendment A (2026-09-18):** the three engine call sites pass `extracted.converter ? { converter: extracted.converter } : undefined` — this is the carriage path for converter provenance into chunk metadata (`metadata_json.converter`).
...
| # | Criterion |
|---|---|
| T1 | `chunkFile(md, "x.pdf", { fileTypeOverride: "markdown", extraMetadata: { converter: "marker" } })` → `chunkMarkdown`-shaped chunks (headings/breadcrumbs) with `file_type: "pdf"` and `metadata.converter` present; `Converter:` line in `buildChunkEmbeddingText`; provenance NEVER materializes without `extraMetadata` being supplied |
