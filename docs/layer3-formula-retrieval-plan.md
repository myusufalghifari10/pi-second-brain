...
- **F5** search: md fixture `$$E = mc^2$$` (+ prose), query `$E=mc^{2}$` ⇒ `match_reason: "formula"`, correct chunk; query without math ⇒ zero `"formula"` reasons and the top chunk id + score equal golden values (deterministic: pinned embedding model + deterministic bm25 — snapshot captured by running the plain query on the fixture and asserting twice within the test).
