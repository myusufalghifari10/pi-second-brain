...
- **The first formula-bearing query after an upgrade triggers a one-time backfill; on large KBs that one query is noticeably slower (once)**. The formula index uses the `knowledge_bases.formula_index_built` flag to guarantee once-per-KB construction: the first formula-bearing query (or first add) after an upgrade synchronously scans every chunk's `metadata_json`, normalizes formulas, and writes the `formulas` table. On a KB with hundreds of thousands of chunks this can take several seconds — expected behavior, not a hang; later queries take the normal path. An interrupted backfill does not write the built flag; it is NOT retried within the same process (the in-process guard is set before the try) — only the first formula-bearing query after a process restart retries it. The backfill is kbId-scoped: it scans only the ensured KB's chunks, so the spike is proportional to that one KB, not the whole database.
