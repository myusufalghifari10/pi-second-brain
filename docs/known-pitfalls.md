...
- **No sidecar installed = silent unpdf fallback (by design, not a bug)**. With `PI_KNOWLEDGE_PDF_ENGINE=auto`, when marker/docling cannot be found the pipeline silently uses unpdf and scientific-PDF formulas stay glyph soup. If you suspect the sidecar is not taking effect, first confirm `pip install marker-pdf` or `pip install docling` succeeded and the binary is on PATH, then check the scan skipped stats for `pdf_sidecar_failed` — only "sidecar detected but conversion failed/timed out" records that reason; simply not having it installed does not.
- **The first marker run downloads GB-scale model weights and is very slow**. The default 120-second timeout may not be enough and the conversion is killed, falling back to unpdf. Fix: raise `PI_KNOWLEDGE_PDF_SIDECAR_TIMEOUT_MS`, or pre-warm by running `marker_single` manually on one PDF before indexing.
- **Conversion is VRAM/CPU heavy**. marker/docling are GPU/RAM-hungry child processes; sidecar conversion is serialized by a mutex so it will not explode in parallel, but watch for resource contention with a local LLM/embedding server while converting large PDFs.
- **Converted PDF chunks carry Markdown content but `file_type` stays `"pdf"`**. This is deliberate filter compatibility: the `file_type: "pdf"` filter covers both sidecar and fallback PDFs; the conversion source is recorded in chunk `metadata_json` as `converter`, and the context prefix gains a `Converter: <name>` line.
- **Markdown image links in converted output point to a deleted temp directory and are dead (inert by design)**. Layer 2 does not extract or store images; dead links are just text and do not affect chunking, FTS, or search — do not "fix" them.
- **Conversion timeout sends SIGTERM only, with no SIGKILL escalation**. A sidecar that ignores SIGTERM may linger; harmless (fail-open, tmpdir cleanup does not throw), but if a real-marker hang is ever reported, check for leftover processes first.
- **`maxBuffer` of 8 MB covers stdout+stderr combined**. A chatty converter (e.g. verbose docling) on a very large PDF can overflow → `output_overflow` → silent unpdf fallback. This is designed behavior, not a failure.

## Formula retrieval (structural-lexical formula index)

- **The first formula-bearing query after an upgrade triggers a one-time backfill; on large KBs that one query is noticeably slower (once)**. The formula index uses the `knowledge_bases.formula_index_built` flag to guarantee once-per-KB construction: the first formula-bearing query (or first add) after an upgrade synchronously scans every chunk's `metadata_json`, normalizes formulas, and writes the `formulas` table. On a KB with hundreds of thousands of chunks this can take several seconds — expected behavior, not a hang; later queries take the normal path. An interrupted backfill does not write the built flag; it is NOT retried within the same process (the in-process guard is set before the try) — only the first formula-bearing query after a process restart retries it. The backfill scans the chunks of all KBs at once (no kbId filter, frozen contract) — the latency/memory spike on the first formula query is more pronounced on large multi-KB databases.
- **Formula equivalence is lexical-structural, not AST/symbolic equivalence**. Cosmetic variants match: `\dfrac{a}{b}` ≡ `\frac{a}{b}`, `\le` ≡ `\leq`, `{x}` ≡ `x`, `\left( x \right)` ≡ `(x)`, `α` ≡ `\alpha`, spacing differences; case is preserved (`E` ≠ `e` — math is case-sensitive). But structurally different yet mathematically equivalent formulas do NOT match: `e^{i\pi} + 1 = 0` vs a reordered `1 + e^{i\pi} = 0`, `x^2 - 1` vs `(x-1)(x+1)`, the same integral written differently — different token sequences do not hit, and there is no SymPy-style simplification. Rely on the chunk-level vector's semantic leg for mathematical equivalence; do not expect the formula leg to hit it.
- **Injected formula results deliberately bypass `MIN_HYBRID_SCORE` (by design, not a gate hole)**. Injected chunks with `match_reason: "formula"` (capped at 5) skip the hybrid confidence gate because their evidence is an exact/FTS formula hit, not a lexical-prose score; raising `PI_KNOWLEDGE_MIN_HYBRID_SCORE` does not block injected results — it only affects normal bm25/vector/hybrid results. The boost is also capped at `FORMULA_BOOST = 0.35`.
- **Backfill failure = formula features silently dormant (fail-open)**. The formula backfill and fusion paths never throw: a single chunk's metadata parse error skips just that chunk, and a whole-body failure only puts formula features to sleep for this process — query and indexing behavior are otherwise unchanged. The cost is no obvious error message; if you suspect formula retrieval is not working, first confirm the query really contains a formula (`$...$`/`$$...$$`/`\[...\]`, or a bare-TeX segment with ≥ 2 TeX commands), then confirm the KB has migrated to schema v6 and its chunks carry `metadata.formulas`. Plain-text queries never trigger the formula path (byte-identical results).
## Layer 4 (chemistry, units, eval harness, images, label graph)

- **`metadata.refs` re-hashes every LaTeX-bearing chunk — one `knowledge_update` re-embeds those files (by design, not a bug)**. `metadata_json` is an input to the chunk identity hash; adding `\ref`-family reference metadata changes the hash of every LaTeX chunk that gains refs, so the first update after upgrading re-chunks and re-embeds LaTeX-bearing files (ADR-020 rebuild-boundary precedent). Non-LaTeX KBs keep identical hashes and reuse their vectors. Do not "optimize" this by excluding refs from the identity hash — that would break incremental-update correctness for any future metadata change.
- **tesseract absent = no OCR captions, silently (fail-open by design)**. The OCR caption path needs a user-installed `tesseract` binary; the availability probe fails open, so caption-less images index without captions instead of failing the PDF. Only caption-less images are OCR'd (images with captions skip OCR for cost). `PI_KNOWLEDGE_OCR_ENGINE=off` hard-disables the path, and `PI_KNOWLEDGE_OCR_CMD` overrides the command (argv template, same substitution rules as `PI_KNOWLEDGE_PDF_SIDECAR_CMD`) for tests or custom binaries. If image queries look caption-blind, first confirm `tesseract --version` works on PATH — there is no error surfaced when it is simply missing.
- **`<knowledge-dir>/image-store/` grows with indexed PDFs; the reset hatch is `rm -rf <knowledge-dir>/image-store`**. Every referenced sidecar image is copied in under its sha256 name, and nothing prunes the store automatically (same posture as `pdf-cache`). The store is derived data, but note the difference from pdf-cache: already-indexed chunk links point into the store, so deleting it dead-links those chunks until the affected PDFs are re-indexed (content change or KB rebuild). Treat wholesale deletion as part of a rebuild, not routine cleanup.
- **`MOLECULAR_RE` is anchored-full-match on purpose — prose molecular formulas are intentionally not indexed**. Index-side chemistry comes only from display-math segments and `\ce{}` inputs; free prose containing `H2SO4` (or words like "NO", "At", "In") is not formula-indexed, because chemistry detection over arbitrary prose is a precision trap that would pollute the formula index. The compensation is on the query side: a plain query segment that full-matches `MOLECULAR_RE` (searching `H2SO4`) is treated as a chemistry formula. Do not add prose-level formula extraction without revisiting that stance; for prose mentions rely on the lexical/BM25 and vector legs.
- **`Nm` is not `N·m` — single-token unit forms are intentionally asymmetric**. The unit rewrite only collapses a middle dot sitting between two recognized unit tokens in the FTS token stream (`N cdot m` → `N m`), because splitting alnum runs is forbidden by the Layer 1 tokenizer invariant. `Nm`, `kgm`, and similar glued forms stay single tokens and do not match `N m`/`N·m`; write units with an explicit separator. Negative guarantee: non-unit pairs like `a·b` are never rewritten.
