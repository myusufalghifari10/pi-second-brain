...
async function embedViaAPI(
	...
	// Validate the response shape: a 200 with an unexpected body (proxy/error JSON, or a literal
	// JSON `null`) must fail with a diagnosable API error instead of "Cannot read properties of
	// null/undefined". The length check matters on the query path: a short/empty data array
	// would surface as an undefined vector and crash the search instead of failing diagnosably.
