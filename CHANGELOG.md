...
### Added
- Added OS-detecting installer scripts (`scripts/install-pi.sh` for Pi; `scripts/install.sh` + its PowerShell twin `scripts/install.ps1` for every MCP harness): clone, run one command, and the script detects Linux/macOS/Windows, checks Node ≥ 22 with per-OS fix hints, installs dependencies with `--ignore-scripts` so third-party install scripts can never break the install, rebuilds `better-sqlite3` alone to fetch its prebuilt binary, builds, and registers into detected harnesses. The README gains a "Connect your coding agent" guide with per-harness wiring and a verification step. Fixed the CLI direct-invocation guard to compare real paths so symlinked npm bins run instead of exiting silently.
