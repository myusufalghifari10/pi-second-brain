...
- Math/table-aware chunking is a behavior contract: display math (`$$`, ```math fences, `\begin{equation}`-family, `\[..\]`), pipe tables, and fenced code are atomic chunking units; `preTokenizeForFTS` canonicalizes math unicode (Greek, operators, sup/subscripts) symmetrically for index and query, and unifies chem formula runs (H 2 O / H sub2 O → H2O, CO 2 → CO2) symmetrically so fast mode matches ASCII and subscript spellings alike; markdown frontmatter (title/tags/aliases) and wikilinks become chunk metadata; `.tex` files are chunked as `latex` with section breadcrumbs and label metadata.
- PDF sidecar is optional and fail-open: marker/docling when available and configured, unpdf fallback otherwise; conversions are cached by content hash; converted PDFs keep file_type pdf and record their converter.
- Formula retrieval is additive and dormant by default: queries without math are byte-identical to the pre-formula behavior; formula-bearing queries boost already-retrieved chunks (FORMULA_BOOST 0.35) and inject up to 5 exact/FTS formula matches (match_reason "formula") that respect kb/filters and bypass MIN_HYBRID_SCORE by design; the per-KB formula index is built once (flag-guarded, fail-open) from chunk metadata formulas via a frozen lexical normalizer (no AST).
- Layer 4 domains are additive and dormant by default: chemistry (\ce{} and molecular formulas via a case-sensitive lexical normalizer routed through the same formula index; prose molecular formulas are intentionally not indexed; Co ≠ CO), engineering units (N·m ≡ N m via a conservative token rewrite in the shared FTS pipeline; Nm stays one token), PDF images (sidecar images persist to a content-addressed image-store with rewritten live links; caption-less images get fail-open tesseract OCR via PI_KNOWLEDGE_OCR_ENGINE/PI_KNOWLEDGE_OCR_CMD), and the label graph (\label/\ref scope-key sha256 resolution, same-file preferred, unresolved edges recorded never guessed, schema v7 label_edges, dependency leg DEPENDENCY_BOOST 0.25 with formula-leg filter/cap/threshold discipline, re-hashing rebuild boundary for LaTeX chunks). `npm run eval` is the byte-deterministic golden-query gate (fixture mode) with advisory live-corpus mode.
