...
describe("latex file type ranking", () => {
	it("normalizes latex file type aliases", () => {
		expect(normalizeFileTypeFilter("tex")).toBe("latex");
		expect(normalizeFileTypeFilter("ltx")).toBe("latex");
		expect(normalizeFileTypeFilter("latex")).toBe("latex");
	});

	it("boosts guide-intent queries on latex documentation identically to markdown", () => {
		const latexChunk = chunk({
			file_path: "docs/paper.tex",
			file_type: "latex",
			content: "Setup guide for the experiment.",
		});
		const markdownChunk = chunk({
			file_path: "docs/paper.md",
			file_type: "markdown",
			content: "Setup guide for the experiment.",
		});
		const queryTokens = tokenizeForSearch("setup guide");

		const latexScore = scoreChunkForQuery(0.5, latexChunk, queryTokens);
		const markdownScore = scoreChunkForQuery(0.5, markdownChunk, queryTokens);

		expect(latexScore.documentation_boost).toBe(0.35);
		expect(markdownScore.documentation_boost).toBe(0.35);
		expect(latexScore.adjusted_score).toBe(markdownScore.adjusted_score);
	});
});
